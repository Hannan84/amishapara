...
class CustomerController extends Controller
{
    ...
    public function signin(Request $request){
        $auth_check_phone = Customer::where('phone',$request->username)->first();
        $auth_check_email = Customer::where('email',$request->username)->first();
        if($auth_check_phone){
            $this->validate($request, [
                'username'    => 'required|numeric',
                'password' => 'required|min:6'
            ]);
            if (Auth::guard('customer')->attempt(['phone' => $request->username, 'password' => $request->password])) {
